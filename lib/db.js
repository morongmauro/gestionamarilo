...
async function updateActivity(id, input) {
  ...
  if (error) fail(error);
  // Si cambiaron fechas o dependencias, las sucesoras se corren solas
  if (fields.start_date !== undefined || fields.deadline !== undefined || fields.depends_on_ids !== undefined) {
    await shiftSuccessors(data.project_id);
  }
  return deriveActivity(data, todayBogota());
}

// ------------------------------------------------------------
// LAS FECHAS SE CORREN SOLAS
// Si A precede a B, B no puede empezar antes de que A termine. Cuando
// hace falta se empuja B (y su cadena) conservando su duración.
// Solo empuja hacia adelante: nunca adelanta una fecha que ya pusiste.
// ------------------------------------------------------------
async function shiftSuccessors(projectId) {
  ...
  function resolver(a) {
    ...
    preds.get(a.id).forEach(pid => {
      ...
      resolver(p);
      if (p.deadline) {
        const siguiente = addDaysISO(p.deadline, 1);
        if (!minimo || siguiente > minimo) minimo = siguiente;
      }
    });
    const inicio = a.start_date || a.deadline;
    if (minimo && inicio && inicio < minimo) {
      const delta = daysBetween(inicio, minimo);
      const start = a.start_date ? addDaysISO(a.start_date, delta) : null;
      const fin = a.deadline ? addDaysISO(a.deadline, delta) : null;
      a.start_date = start;
      a.deadline = fin;
      cambios.push({ id: a.id, start_date: start, deadline: fin });
    }
    estado[a.id] = 'listo';
  }
