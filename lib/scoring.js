...
// (motor de priorización, independiente de la fuente de datos)
// ============================================================
const DAY = 86400000;
...
// Redacción del porqué una tarea es foco del día
function buildFocoReason(t) {
  const parts = [];
  if (t.dueDate) {
    const d = Math.round((new Date(t.dueDate + 'T12:00:00').setHours(0, 0, 0, 0) - new Date().setHours(0, 0, 0, 0)) / DAY);
    if (d < 0) parts.push(`venció hace ${Math.abs(d)} día${Math.abs(d) === 1 ? '' : 's'} y sigue abierta`);
    else if (d === 0) parts.push('vence hoy');
    else if (d === 1) parts.push('vence mañana');
    else if (d <= 3) parts.push(`vence en ${d} días`);
  }
  if (t.priority === 'High') parts.push('la marcaste como prioridad alta');
  if (t.tipoGestion === 'Depende Tercero') {
    if (t.stagDays >= 5) parts.push(`lleva ${t.stagDays} días esperando a un tercero sin movimiento — toca presionar hoy`);
    else parts.push('depende de un tercero: un empujón hoy evita que se estanque');
  } else if (t.tipoGestion === 'Compartida') {
    parts.push('requiere coordinación con otros y eso siempre toma más tiempo del previsto');
  }
  if ((t.ice ?? 0) >= 70) parts.push(`su ICE de ${t.ice} indica alto impacto para el negocio`);
  if (t.effortLevel === 'Small') parts.push('es un cierre rápido que libera espacio mental');
  if (!parts.length) {
    return `Es la tarea con el score de priorización más alto en este momento (${t.score} pts)${t.topic ? ` dentro de ${t.topic}` : ''}: cerrarla hoy mantiene el momentum.`;
  }
  const sentence = parts.join('; ');
  return sentence.charAt(0).toUpperCase() + sentence.slice(1) + `. Score de priorización: ${t.score}.`;
}

function buildBandejas(todo) {
  const enriched = todo.map(enrich);
  const sorted = [...enriched].sort((a, b) => b.score - a.score);
  const focos = sorted.slice(0, 3).map(t => ({ ...t, focoReason: buildFocoReason(t) }));
  const foco = focos[0] ?? null;
